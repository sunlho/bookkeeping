...
class GroupItem {
  ...
  GroupItem({required this.icon, required this.type, required this.desc, required this.checked});
}
...
class EditGroupState extends State<EditGroupPage> {
  ...
  List<GroupItem> _groupItems = MethodsIcons.circleTypes.map((item) {
    return GroupItem(checked: false, icon: item.icon, desc: item.desc, type: item.type);
  }).toList();
  ...
  _editGroup() async {
    ...
    bool editSuccess = await groupStore.editGroup({'id': widget.id, 'name': _nameController.text, 'type': _selected?.type, 'usage': '1', 'isDefault': _switchValue ? '1' : '0', 'desc': ''});
    if (editSuccess) {
      context.pop();
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(title: Text('编辑记账圈子', style: TextStyle(fontSize: Adaptor.px(32.0), color: AppColors.appTextDark))),
        body: Container(
              ...
              children: <Widget>[
                Container(
                  ...
                  child: Swiper(
                      ...
                      itemBuilder: (context, index) {
                        List _subRow = _groupItems.sublist((index * 8), min((index + 1) * 8, MethodsIcons.circleTypesLength));
                        return Container(
                            child: Wrap(
                          ...
                          return Container(
                            ...
                            child: ElevatedButton(
                              onPressed: () {
                                ...
                                setState(() {
                                  _currentIndex = index;
                                  _groupItems.asMap().keys.forEach((int cur) {
                                    if (cur == position) {
                ...
                Container(
                  ...
                  child: Row(
                    ...
                    children: <Widget>[
                      Text('圈子名称', style: TextStyle(color: AppColors.appTextDark, fontSize: Adaptor.px(28.0))),
                      Expanded(
                        flex: 1,
                        child: Container(
                          child: TextField(
                              decoration: InputDecoration(contentPadding: EdgeInsets.only(right: Adaptor.px(6.0)), hintText: '请输入圈子名称', fillColor: Colors.transparent, filled: true, border: InputBorder.none),
                              autocorrect: false,
                ...
                Container(
                  ...
                  child: Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: <Widget>[
                      Text('设为默认', style: TextStyle(color: AppColors.appTextDark, fontSize: Adaptor.px(28.0))),
                      Container(
