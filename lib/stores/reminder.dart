...
abstract class _ReminderStore extends BaseStore with Store {
  ...
  Future<bool> queryReminder([bool toast = false]) async {
    try {
      ...
      if (data.success) {
        reminder = [];
        data.data.toList().forEach((json) => {reminder.add(new ReminderItem.fromJson(json))});
      }
  ...
  Future<bool> getDetail(String id) async {
    try {
      ...
      Map<String, dynamic> resp = await HttpUtil.request(Api.reminderDetail, {'id': id}, HttpUtil.GET);
