...
abstract class _GroupStore extends BaseStore with Store {
  ...
  Future<bool> createGroup(Map<String, dynamic> group) async {
    try {
      ...
      Map<String, dynamic> resp = await HttpUtil.request(Api.createGroup, group, HttpUtil.POST);
  ...
  Future<bool> editGroup(Map<String, dynamic> group) async {
    try {
      switchLoading(true);
      Map<String, dynamic> resp = await HttpUtil.request(Api.updateGroup, group, HttpUtil.PUT);
  ...
  Future<bool> queryGroups([bool toast = false]) async {
    try {
      ...
      Map<String, dynamic> resp = await HttpUtil.request(Api.queryGroup, {}, HttpUtil.GET);

      HttpResponse data = new HttpResponse.formJson(resp);

      if (data.success) {
        groups = [];
        data.data.toList().forEach((json) => {groups.add(new GroupItem.fromJson(json))});
      }
  ...
  Future<bool> deleteReminder(String id) async {
    try {
      ...
      Map<String, dynamic> resp = await HttpUtil.request(Api.deleteReminder, {'id': id}, HttpUtil.DELETE);
  ...
  Future<bool> getDetail(String id) async {
    try {
      ...
      Map<String, dynamic> resp = await HttpUtil.request(Api.groupDetail, {'id': id}, HttpUtil.GET);
