...
class RemindersState extends State<RemindersPage> with WidgetsBindingObserver {
  ...
  void _toCreateReminder() {
    context.go('/reminder-create');
  }

  void _toDetail(int id) {
    context.go('/reminder-detail', extra: {
      'id': id,
    });
  }
  ...
  Widget _buildReminders() {
    return Observer(
      builder: (_) => Container(
        ...
        child: (reminderStore.reminder.length > 0)
            ? SingleChildScrollView(
                ...
                child: Wrap(
                    children: List.generate(reminderStore.reminder.length, (int i) {
                                  ...
                                  children: <Widget>[
                                    Row(
                                      ...
                                      children: <Widget>[
                                        ...
                                        Padding(padding: EdgeInsets.only(left: Adaptor.px(10.0)), child: Text('存钱提醒', style: TextStyle(fontSize: Adaptor.px(28.0), color: AppColors.appTextDark))),
                                        Text(
                                          Util.isAfter(_reminder.time) ? '(已提醒)' : '',
                                          style: TextStyle(fontSize: Adaptor.px(20.0), color: AppColors.appIncome),
                                        )
                                      ],
                                    ),
                                    Text('${Util.frenquency2Weekday(_reminder.frequency)} ${_reminder.time}', style: TextStyle(fontSize: Adaptor.px(26.0), color: AppColors.appTextDark))
                                  ],
                                ),
                              ),
                              Container(
                                padding: EdgeInsets.all(Adaptor.px(10.0)),
                                child: Text(
                                  _reminder.rule == '0' ? '金额每天递增' : '金额每天固定',
                                  style: TextStyle(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('存钱提醒', style: TextStyle(fontSize: Adaptor.px(32.0), color: AppColors.appTextDark))),
      body: Container(
