...
class TypeItem {
  ...
  TypeItem({required this.icon, required this.type, required this.desc, required this.checked});
}
...
class RecordState extends State<RecordPage> with SingleTickerProviderStateMixin {
  ...
  String _billDate = DayDart().format('YYYY-MM-DD');
  ...
  void initState() {
    ...
    MethodsIcons.paymentIcons.forEach((item) {
      _payTypes.add(TypeItem(icon: item.icon, type: item.type, desc: item.desc, checked: false));
    });

    MethodsIcons.incomeIcons.forEach((item) {
      _incomeTypes.add(TypeItem(icon: item.icon, type: item.type, desc: item.desc, checked: false));
    });
  }
  ...
  void _handleTabChange() {
    ...
    if (_tabController.indexIsChanging) {
      setState(() {
        ...
        StatusBarControl.setColor(_mainColor);
  ...
  void _showDatePicker(String type) async {
    ...
    DateTime newDateTime = await showRoundedDatePicker(
      ...
      borderRadius: 0,
      theme: ThemeData(primarySwatch: _currentIndex == 0 ? AppColors.appYellowMaterial : AppColors.appGreenMaterial),
      locale: Locale('zh', 'CN'),
    ) as DateTime;

    setState(() {
      _billDate = DayDart(newDateTime).format('YYYY-MM-DD');
    });
  }
      ...
      builder: (context) {
        return StatefulBuilder(builder: (BuildContext context, StateSetter setState) {
          return Container(
              ...
              decoration: BoxDecoration(color: AppColors.appWhite, borderRadius: BorderRadius.only(topLeft: Radius.circular(Adaptor.px(40.0)), topRight: Radius.circular(Adaptor.px(40.0)))),
              padding: EdgeInsets.only(top: Adaptor.px(40.0), bottom: Adaptor.px(20.0), left: Adaptor.px(10.0)),
              child: Wrap(children: <Widget>[
                Container(
                  child: Center(child: Text('选择付款方式', style: TextStyle(fontSize: Adaptor.px(32.0), fontWeight: FontWeight.w400, color: AppColors.appTextDark))),
                ),
                Wrap(
                    children: List.generate(_payChannels.length, (int index) {
                  return Container(
                      child: RadioListTile(
                          title: Text(_payChannels[index].desc, style: TextStyle(fontSize: Adaptor.px(32.0), color: AppColors.appTextDark)),
                          activeColor: AppColors.appYellow,
                          value: index,
                          groupValue: _payChannelIndex,
                          onChanged: (int? value) {
                            setState(() {
                              _payChannelIndex = value ?? 0;
                            });
                          }));
                }).toList()),
                Container(
                    padding: EdgeInsets.only(left: Adaptor.px(30.0), right: Adaptor.px(46.0)),
                    margin: EdgeInsets.only(top: Adaptor.px(30.0)),
                    child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: <Widget>[
                      GestureDetector(
                        onTap: () => {Navigator.of(context).pop()},
                        child: Container(
                            decoration: BoxDecoration(color: AppColors.sheetBtnBg, borderRadius: BorderRadius.all(Radius.circular(Adaptor.px(45.0)))),
                            width: Adaptor.px(280.0),
                            height: Adaptor.px(90.0),
                            child: Center(
                                child: Text(
                              '取消',
                              style: TextStyle(fontSize: Adaptor.px(32.0), color: AppColors.appDanger),
                            ))),
                      ),
                      GestureDetector(
                        ...
                        child: Container(
                            ...
                            child: Center(
                                child: Text(
                              '确定',
                              style: TextStyle(fontSize: Adaptor.px(32.0), color: AppColors.appWarning),
                            ))),
                      )
                    ]))
              ]));
  ...
  void _groupSelectOk() {
    setState(() {
      if (_groupIndex != null) _groupItem = _groups[_groupIndex!];
    });
    Navigator.of(context).pop();
  }
      ...
      builder: (context) {
        return StatefulBuilder(builder: (BuildContext context, StateSetter setState) {
          return Container(
              ...
              child: Wrap(children: <Widget>[
                Container(
                  child: Center(child: Text('选择记账圈子', style: TextStyle(fontSize: Adaptor.px(32.0), fontWeight: FontWeight.w400, color: AppColors.appTextDark))),
                ),
                Container(
                  ...
                  child: SingleChildScrollView(
                    ...
                    return Container(
                        child: RadioListTile(
                            title: Text(_groups[index].name, style: TextStyle(fontSize: Adaptor.px(32.0), color: AppColors.appTextDark)),
                            activeColor: AppColors.appYellow,
                            value: index,
  ...
  @override
  void dispose() {
    StatusBarControl.setColor(AppColors.appWhite);
  ...
  void _createBill(type) async {
    ...
    if (type == 'income') {
      ...
      _bill = {'amount': _incomeInputController.text, 'billType': '1', 'category': _category?.type, 'billDate': _billDate, 'remark': _payMarkController.text};
    } else {
      ...
      _bill = {'amount': _paymentInputController.text, 'billType': '0', 'category': _category?.type, 'billDate': _billDate, 'groupId': _groupItem?.id, 'payMethod': _selectedChannel.type, 'remark': _payMarkController.text};
    }
  ...
  Widget _buildPayment(BuildContext context) {
    return SingleChildScrollView(
      ...
      children: <Widget>[
          ...
          child: TextField(
              ...
              decoration: InputDecoration(
                  ...
                  enabledBorder: UnderlineInputBorder(
                    borderSide: BorderSide(color: AppColors.appTextDark, width: Adaptor.onePx()),
                  ),
                  focusedBorder: UnderlineInputBorder(borderSide: BorderSide(color: AppColors.appTextDark, width: Adaptor.onePx()))),
        ...
        Container(
            ...
            child: Swiper(
                ...
                itemBuilder: (context, index) {
                  List _subRow = _payTypes.sublist((index * 8), min((index + 1) * 8, MethodsIcons.paymentLength));
                  return Container(
                      child: Wrap(
                        ...
                        child: Container(
                            ...
                            child: Column(mainAxisAlignment: MainAxisAlignment.center, children: <Widget>[
                              Container(
                                  ...
                                  margin: EdgeInsets.only(bottom: Adaptor.px(10.0)),
                                  decoration: BoxDecoration(color: _item.checked ? AppColors.appYellow : AppColors.appBorder, borderRadius: BorderRadius.all(Radius.circular(Adaptor.px(50)))),
                                  child: Icon(
                                    _item.icon,
                                    size: Adaptor.px(50.0),
                                    color: _item.checked ? AppColors.appWhite : AppColors.appTextDark,
                                  )),
                              Text(_item.desc, style: TextStyle(fontSize: Adaptor.px(24.0), color: AppColors.appTextDark))
                            ])));
                  }).toList()));
                },
                itemCount: (MethodsIcons.paymentLength / 8).ceil(),
                pagination: SwiperPagination(alignment: Alignment.bottomCenter, builder: RectSwiperPaginationBuilder(color: AppColors.appBorder, activeColor: AppColors.appYellow, size: Size(18, 3), activeSize: Size(18, 3), space: 0.2)),
                autoplay: false,
        ...
        Container(
          padding: EdgeInsets.only(top: Adaptor.px(16.0), bottom: Adaptor.px(16.0), left: Adaptor.px(20.0), right: Adaptor.px(32.0)),
          ...
          child: Flex(direction: Axis.horizontal, mainAxisAlignment: MainAxisAlignment.spaceBetween, crossAxisAlignment: CrossAxisAlignment.center, children: <Widget>[
            Container(
                child: Text(
              '付款方式',
              style: TextStyle(
            ...
            Expanded(
              child: GestureDetector(
                ...
                child: Align(
                  ...
                  child: Text(
                    _selectedChannel != null ? _selectedChannel.desc : '请选择付款方式',
                    style: TextStyle(color: AppColors.appTextDark, fontSize: Adaptor.px(28.0), fontWeight: FontWeight.normal),
                  ),
                ),
              ),
            )
            ...
            Expanded(
              child: GestureDetector(
                ...
                child: Align(
                  alignment: Alignment.centerRight,
                  child: Text(
                    _groupItem?.name != null ? _groupItem!.name : '请选择记账圈子',
                    style: TextStyle(color: AppColors.appTextDark, fontSize: Adaptor.px(28.0), fontWeight: FontWeight.normal),
                  ),
                ),
              ),
        ...
        Container(
          ...
          child: Flex(direction: Axis.horizontal, mainAxisAlignment: MainAxisAlignment.spaceBetween, crossAxisAlignment: CrossAxisAlignment.center, children: <Widget>[
            ...
            Expanded(child: GestureDetector(onTap: () => _showDatePicker('pay'), child: Align(alignment: Alignment.centerRight, child: Text(_billDate, style: TextStyle(color: AppColors.appTextDark, fontSize: Adaptor.px(28.0), fontWeight: FontWeight.normal)))))
          ]),
        ),
        Container(
          padding: EdgeInsets.only(left: Adaptor.px(20.0), right: Adaptor.px(32.0)),
          height: Adaptor.px(100.0),
          decoration: BoxDecoration(
              border: Border(
            bottom: BorderSide(width: Adaptor.onePx(), color: AppColors.appBorder),
            ...
            Expanded(
              child: Container(
                ...
                child: TextField(
                  ...
                  decoration: InputDecoration(hintText: '请输入账单备注', fillColor: Colors.transparent, contentPadding: EdgeInsets.symmetric(vertical: 10.0), filled: true, border: InputBorder.none),
                  style: TextStyle(
                    fontSize: Adaptor.px(28.0),
                    color: AppColors.appTextDark,
        ...
        GestureDetector(
          ...
          child: Container(
            ...
            padding: EdgeInsets.only(left: Adaptor.px(16.0), right: Adaptor.px(16.0)),
            margin: EdgeInsets.only(top: Adaptor.px(40.0), left: Adaptor.px(10.0), right: Adaptor.px(10.0)),
            decoration: BoxDecoration(color: AppColors.appYellow, borderRadius: BorderRadius.all(Radius.circular(Adaptor.px(10.0)))),
            child: Center(
              child: Text(
                '记一笔',
                style: TextStyle(fontSize: Adaptor.px(30.0), fontWeight: FontWeight.normal, color: AppColors.appTextDark),
              ),
            ),
          ),
        ),
      ...
      children: <Widget>[
        ...
        Container(
            ...
            child: Wrap(
              children: <Widget>[
                Row(
                  children: List.generate(MethodsIcons.incomeRowLengthMax, (int i) {
                        ...
                        child: Column(
                          ...
                          children: <Widget>[
                            Container(
                                ...
                                margin: EdgeInsets.only(bottom: Adaptor.px(10.0)),
                                decoration: BoxDecoration(
                                  color: _item.checked ? AppColors.appGreen : AppColors.appBorder,
  ...
  Widget build(BuildContext context) {
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: Text('记账', style: TextStyle(fontSize: Adaptor.px(32.0), color: AppColors.appTextDark)),
          backgroundColor: _mainColor,
          bottom: TabBar(
            controller: _tabController,
            indicatorColor: Colors.transparent,
            tabs: <Widget>[
              Tab(child: Text('支出', style: TextStyle(fontSize: Adaptor.px(_currentIndex == 0 ? 36.0 : 32.0), fontWeight: _currentIndex == 0 ? FontWeight.w500 : FontWeight.w400))),
              Tab(
                child: Text(
                  '收入',
                  style: TextStyle(fontSize: Adaptor.px(_currentIndex == 1 ? 36.0 : 32.0), fontWeight: _currentIndex == 1 ? FontWeight.w500 : FontWeight.w400),
                ),
              ),
            ],
          ),
